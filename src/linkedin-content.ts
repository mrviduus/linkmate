...
class LinkedInLinkMate {
   ...
   * isn't on LinkMate's own injected UI), ping the side panel so it can fade
   * itself out. Throttled; harmless no-op when the panel isn't open. The panel
   * ignores pings within its open grace window, so the gesture that opened it
